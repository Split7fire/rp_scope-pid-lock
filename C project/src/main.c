...
static rp_app_params_t rp_main_params[PARAMS_NUM+1] = {
    ...
    { /* pid_NN_kd - PID NN derivative gain   Kd in [ADC] counts. */
        ...
        "lockin_phase",  0, 1, 0, -8192, 8191 },
    { /* */
        "lockin_gain",  0, 1, 0, -8192, 8191 },
    { /*
           *    0 - PID disabled (open loop)
           *    1 - PID enabled (closed loop)    */
        "lockin_pid1_enable", 0, 1, 0, 0, 1 },
    { /* */
        "lockin_pid1_sp",  0, 1, 0, -8192, 8191 },
    { /* */
        "lockin_pid1_kp",  0, 1, 0, -8192, 8191 },
    { /* */
        "lockin_pid1_ki",  0, 1, 0, -8192, 8191 },
    { /* */
        "lockin_pid1_kd",  0, 1, 0, -8192, 8191 },
    { /*
           *    0 - PID disabled (open loop)
           *    1 - PID enabled (closed loop)    */
        "lockin_pid2_enable", 0, 1, 0, 0, 1 },
    { /* */
        "lockin_pid2_kp",  0, 1, 0, -8192, 8191 },
    { /* */
...
{
    return (const char *)"Red Pitaya lockin application.\n";
}

int rp_app_init(void)
{
    fprintf(stderr, "Loading scope (with gen+pid+lockin extensions) version %s-%s.\n", VERSION_STR, REVISION_STR);
...
{
    fprintf(stderr, "Unloading scope (with gen+pid+lockin extensions) version %s-%s.\n", VERSION_STR, REVISION_STR);
...
int rp_set_params(rp_app_params_t *p, int len)
{
    ...
    for(i = 0; i < len || p[i].name != NULL; i++) {
        ...
        if(rp_main_params[p_idx].value != p[i].value) {
            ...
            if ( (p_idx >= PARAMS_AWG_PARAMS) && (p_idx < PARAMS_PID_PARAMS) )
                awg_params_change = 1;
            if ( (p_idx >= PARAMS_PID_PARAMS) && (p_idx < PARAMS_LOCKIN_PARAMS) )
                pid_params_change = 1;
            if (p_idx >= PARAMS_LOCKIN_PARAMS)
                lockin_params_change = 1;
    ...
    if (lockin_params_change) {
        if(lockin_update(&rp_main_params[0]) < 0) {
            return -1;
        }
    }
